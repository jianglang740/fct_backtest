# -*- coding: utf-8 -*-
"""
加密货币单因子回测——复用公司原版框架 fct_backtest无冗余版.py 的全部逻辑。

与原版的唯一差异（仅数据加载段）：
  1. 数据源切换为 crypto_price_data.parquet / crypto_fct_df.parquet（repo 根目录）；
  2. 加密无拆股分红，回测前注入 close_adj = close（原版 factor_analysis 依赖 close_adj 计算收益）；
  3. 加密货币不做市值 / 行业中性化，数据中无 mkt_cap / industry 列；
  4. 遍历 crypto_fct_df.parquet 的全部因子列，每个因子一份 Excel + 两张图。

其余 MAD 缩尾 / Z-score 标准化 / 样本池过滤（去 is_st、trade_days<365）/
非重叠分层回测 / IC-ICIR / 多空收益 / 组合指标 / Excel 三 Sheet / 图表
均与公司原版 fct_backtest无冗余版.py 逐字一致。

注意：原版年化按 A 股 252 交易日口径；加密货币 7×24 连续交易（一年约 365 个自然日），
年化收益率 / 夏普会系统性偏低，属原版框架口径，此处保留不改。
"""
import os
import pandas as pd
import numpy as np
...
DATA_FCT = os.path.join(ROOT, 'crypto_fct_df.parquet')
OUT_FIGS_DIR = os.path.join(ROOT, 'crypto_图片')
...
###############################################################################
#### 以下函数与 fct_backtest无冗余版.py 逐字一致（原版框架逻辑） ####
###############################################################################
...
def factor_analysis(factor_series, price_df, periods=(1, 5, 10), quantiles=10):
    results = {}
    price_aligned = pd.pivot_table(price_df['close_adj'].reset_index(),
                                   index='trade_date', columns='code', values='close_adj')
...
def analyze_factor_performance(res_):
    ...
    for period, df in res_.items():
        ...
        quantile_returns = pd.pivot_table(quantile_returns, index='trade_date',
                                          columns='quantile', values=f'return_{period}d')
...
def plot_quantile_returns_separate(perform_, factor_name=''):
    ...
    for period in periods:
        ...
        plt.title(f'{period}天 - 分层收益 ({factor_name})', fontsize=16, fontweight='bold')
        ...
        plt.savefig(os.path.join(OUT_FIGS_DIR, f'{factor_name}_quantile_{period}d.png'),
                    dpi=150, bbox_inches='tight')
        plt.show()


def plot_factor_performance(perfor_, factor_name=''):
    ...
    plt.tight_layout()
    os.makedirs(OUT_FIGS_DIR, exist_ok=True)
    plt.savefig(os.path.join(OUT_FIGS_DIR, f'{factor_name}_factor_performance.png'),
                dpi=150, bbox_inches='tight')
    plt.show()


###############################################################################
#### 数据加载与回测（仅此段与原版不同：切换 crypto 数据源 + 注入 close_adj） ####
###############################################################################

def main():
    print('=' * 60)
    print('加密货币单因子回测（复用公司原版框架，无中性化）')
    print('=' * 60)

    price_data = pd.read_parquet(DATA_PRICE)
    fct_df = pd.read_parquet(DATA_FCT)
    # 两表索引均去 UTC 时区（仅 trade_date 层）：否则 join 对不上、Excel 也无法写日期
    for _df in (price_data, fct_df):
        _i = _df.index
        _df.index = _i.set_levels(_i.levels[0].tz_localize(None), level=0)
    price_data['close_adj'] = price_data['close']   # 加密无复权：复权价 = 原价
    print(f'合并前价格数据: {price_data.shape}')

    factor_names = list(fct_df.columns)
    price_data = pd.concat([price_data, fct_df], axis=1, join='inner')
    print(f'合并后数据: {len(price_data):,} 行 × {price_data.shape[1]} 列（因子: {factor_names}）')

    # 因子预处理：全局 MAD 缩尾 + Z-score 标准化（原版口径）
    ...
    # 样本池过滤：剔除 is_st、上市不足 365 天
    price_data = preprocess_data(price_data)
    print(f'预处理后: {len(price_data):,} 行（剔除 is_st / 不足 365 天，全局 MAD+Z-score）')

    all_summaries = []
    for factor in factor_names:
        print(f'\n{"=" * 50}\n回测因子: {factor}\n{"=" * 50}')
        factor_df = price_data[factor]
        ...
        out_xlsx = os.path.join(ROOT, f'crypto_因子回测统计结果_{factor}.xlsx')
        summary_df = generate_summary_statistics(performance, out_xlsx)

        if not summary_df.empty:
            summary_df.insert(0, '因子', factor)
            all_summaries.append(summary_df)
            print("\n=== 详细统计结果汇总 ===")
            print(summary_df.to_string(index=False, float_format='%.4f'))

        plot_factor_performance(performance, factor)
        plot_quantile_returns_separate(performance, factor)

    if all_summaries:
        combined = pd.concat(all_summaries, ignore_index=True)
        combined_path = os.path.join(ROOT, 'crypto_因子回测统计结果_汇总.xlsx')
        with pd.ExcelWriter(combined_path, engine='openpyxl') as writer:
            combined.to_excel(writer, sheet_name='汇总统计', index=False)
        print(f'\n全部因子汇总已保存到: {combined_path}')
